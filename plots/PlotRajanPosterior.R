...
###############################################################################
### Load the MCMC results and mesh it.
###############################################################################
## load("rajan_pri_eqspace_var2n_s_moving_20_plus_a_moving_4_2012Aug14-18.37.49.Rdata")
...
rm(y.orig0, Y.mesh)
ofile <- paste("Plot_", ModelDescription, sep = "")
...
init.knots.s <- (INIT.knots[["thinplate.s"]] +
                 meanRaw.s[1:q.s, , drop = FALSE])*sdRaw.s[1:q.s, , drop = FALSE]
init.knots.a <- (matrix(INIT.knots[["thinplate.s"]], q.a1, 2) +
                 meanRaw.s[1:q.a1, , drop = FALSE])*sdRaw.s[1:q.a1, , drop = FALSE]
...
filled.contour(market2book, profit, cnt, xlim = xlim0, ylim = ylim0, col = color0,
               ...
               plot.axes = {axis(1, at = x.at)
                            ...
                            image(cont.y$x, cont.y$y,
                                  ...
                                  add = TRUE, axes = FALSE)
                            points(init.knots.a[1:3, 1],rep(-3.6, 3),  pch =
                            ...
                            points(init.knots.a[4, 1]-0.3,rep(-3.6, 1),  pch =
                                   18, col = "blue", cex = 1.0, xpd = NA)
                            points(rep(-3.65, 4), init.knots.a[, 2],  pch = 18,
                                   col = "white", cex = 1.5)
                            points(rep(-3.65, 4), init.knots.a[, 2],  pch = 18,
                                   col = "blue", cex = 1.0, xpd = NA)
                            legend(x = 20, y = -2.5, pch = c(20, 18), col =
...
filled.contour(X1, X2, y.origmean, col = color0, xlim = xlim1, ylim = ylim1,
               levels = seq(min(y.origmean),
                 max(y.origmean), length.out = nlev+1),
               xlab = "Market2Book", ylab = "Profit",
...
filled.contour(X1, X2, y.origsd, col = color0, xlim = xlim1, ylim = ylim1,
               levels = seq(min(y.origsd), max(y.origsd), length.out = nlev+1),
               xlab = "Market2Book",
               ylab = "Profit",
