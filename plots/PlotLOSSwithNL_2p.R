...
title(main = "Low nonlinearity", ylab = expression(log*frac(Loss(q[s])[fixed], Loss(q[s] == 5)[free])), xpd = NA)
grid2(y.at = ygrid)
grid2(y.at = 0, col = "red", lwd = 1.5, lty = "dashed")
...
axis(2, yat)
title(ylab = expression(log*frac(Loss(q[s])[fixed], Loss(q[s] == 10)[free])), xpd = NA)
grid2(y.at = ygrid)
grid2(y.at = 0, col = "red", lwd = 1.5, lty = "dashed")
