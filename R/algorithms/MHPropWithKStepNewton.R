                                  ...
                                  splineArgs, priorArgs, prop.df, Params_Transform) 
{
  ...
  KStepNewton1 <- KStepNewtonMove(param.cur = param.cur, gradhess.fun.name =
                                  gradhess.fun.name, KStep = nNewtonStep, Params = Params,
                                  hessMethod = hessMethod, Y = Y, x = x, callParam =
                                  callParam, splineArgs = splineArgs, priorArgs = priorArgs, 
  ...
  if(any(is.na(HessObs.cur.prop)) || is(try(chol(-invHessObs.cur.prop), silent=T), "try-error")) # Something is wrong,  reject it.
    {
  ...
  if (any(is.na(param.prop))) # Previous proposal unsuccessful
    ...
    {  
      KStepNewton2 <- KStepNewtonMove(param.cur = param.prop, gradhess.fun.name =
                                      gradhess.fun.name, KStep =nNewtonStep, Params =
                                      Params,hessMethod = hessMethod, Y = Y, x = x, callParam
                                      = callParam, splineArgs = splineArgs, priorArgs =
                                      priorArgs, Params_Transform = Params_Transform)  
