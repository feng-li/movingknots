...
##----------------------------------------------------------------------------------------
      ...
      gradObs.tmp2.0 <- Mat.x.AT.k.I.x.K.x.delta.knots(Mat = Sigma4beta.tilde, A =
                                                       Y.Sigma.inv, delta.knots, p, q,
                                                       q.i) # PASSED
      ...
      if(all(mu == 0)) # gradient for beta can be simplified
      ...
      else # no way
        {
          aT1 <- t(mu)
          B1 <- diag1(p*q)
          aT2 <- crossprod((XT.Y.Sigma.inv + Sigma4beta.inv %*% mu), Sigma4beta.tilde)
          B2 <- Sigma4beta.tilde
          aT2.k.B2 <- aT2 %x% B2
...
##----------------------------------------------------------------------------------------
      ...
      if(all(mu == 0)) # Gradient for beta can be simplified
      ...
      else # bad luck.
        {
          ...
          gradObs.tmp2.3.4 <- Sigma4beta.tilde %*% (t(mu) %x% diag1(p*q))
