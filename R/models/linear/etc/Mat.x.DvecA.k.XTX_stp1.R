...
Mat.x.DvecA.k.XTX_stp1 <- function(Mat, A, p, q) 
{
  ...
  idx4Mat <- matrix(array(1:(p^2*q^2), c(q, p, p*q))[, , as.vector(matrix(1:(p*q), p, q,
                    byrow = TRUE))], , q, byrow = TRUE)  

  idx4Mat2 <- foldMat(idx4Mat, nfolds = q, byrow = TRUE)
  
  Mat2 <- array(Mat[, as.vector(idx4Mat2)], c(dim.Mat[1], p^2, q^2)) # Reorder and convert
                                        # to an array 
