...
x.testing <- M3.features[M3.features[, 13] == 12, c(1:10, 12, 14, 16:20)]
...
M3MASEoutReal <- M3MASEout[M3.features[, 13] == 12, ]
## M3MASEoutMin <- apply(M3MASEoutReal, 1, min)
## M3MASEoutMin <- rowMins(M3MASEoutReal)
M3MASEOpt.monthly <- rep(NA, length(M3.fm))
for (i in 1: length(M3.fm)){
    M3MASEOpt.monthly[i] <- M3MASEoutReal[i, M3.fm[i]]}
