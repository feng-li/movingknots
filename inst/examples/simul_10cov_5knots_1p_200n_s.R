...
## ARGUMENTS FOR SPLINES GENERATING
splineArgs.gen <- list(comp = c("intercept", "covariates", "thinplate.s"), # the components of the design matrix.
                   thinplate.s.dim = c(q.s_gen, q.o)) # the dimension of the knots for surface.

splineArgs.ctrl <- list(comp = c("intercept", "covariates", "thinplate.a"), # the components of the design matrix.
                   thinplate.a.locate = rep(1, q.o)) # the dimension of the knots for surface.
...
ModelDescription <- "simul_10cov_5knots_1p_200n_s"
...
for(iRep in 1:totalRep)
{
  ...
  ## If generate New dataset
  if(iRep  %in% seq(1, totalRep, by = oneRep.len)) # Generate new data
    {
      idx4Data <- idx4Data + 1

      ## Generate new dataset
      Data.gen <- DGP.surface(n = n, p = p, q.o = q.o, q.s = q.s_gen, Sigma.gen,
                              splineArgs.gen, splineArgs.ctrl)

      OUT.Data.gen[[idx4Data]] <- Data.gen
      OUT.NolinFct[[idx4Data]] <- Data.gen$NonlinFactor
      x <- Data.gen$x
      Y <- Data.gen$Y
      x.testing <- Data.gen$x.testing
    }
...
## Posterior analyses
##----------------------------------------------------------------------------------------
  MovingKnots_Dignosis <- FitDiagnosis(x = x.testing, Y = NA, OUT.Params, Data.gen,
                                       logpost.fun.name, splineArgs, splineArgs.gen,
