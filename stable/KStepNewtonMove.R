...
KStepNewtonMove <- function(param.cur, gradhess.fun.name, KStep, Params, hessMethod, Y, x,
                            callParam, splineArgs, priorArgs, Params_Transform) 
{
  ...
  for(k in 1:(KStep+1))
    {
      ...
      if(is(invHessObs.cur, "try-error")) # bad,  return NaN and quit, this draw will be
