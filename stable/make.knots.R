...
make.knots <- function(x, method, splineArgs)
{
  ...
  ## Selection knots locations based on different methods
  if(tolower(method) == "no-knots") # ad-hoc for regression without knots
    ...
    {
      ## Equal spaced sample quantile for a single covariate.
      ## In the multivariate case,  we assume the covariates are independent at
      ## the moment
      ## TODO: use a multivariate quantile function.

      ## x should be a vector and n.locations is a positive integer
      uniquantile <- function(data, n.locations)
        {
          probs <- seq(0,1,length.out=n.locations+2)[-c(1,n.locations+2)]
          xi <- quantile(data, probs)
          return(xi)
        }
      ...
      if("thinplate.a"  %in% splineArgs$comp)
        {
          ...
          x.noempty <- x[, a.locate!=0, drop = FALSE]
          x.noempty.list <- array2list(x.noempty, 2)
          location <-  mapply(FUN = uniquantile,
                              data = x.noempty.list,
                              n.locations = as.list(nknots))
          out[["thinplate.a"]] <- matrix(unlist(location))
        }
    }
  else
