...
linear_IWishart <- function(Params, Y, x, splineArgs, priorArgs, Params_Transform)
{
  ...
  for(i in 1:length(q.i))
    {
      ...
      if(P.type[i] == "identity") # Sparse 
          {
            G.tilde.i <- K.i2 %d*% crossprod(D.i) %*d% K.i2
          }
        else # not sparse
          {
            G.tilde.i <- K.i2 %d*% crossprod(D.i, P.i) %*% D.i %*d% K.i2
          }
